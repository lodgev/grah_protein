...
class ProteinGraphQuery:
    ...
    def get_full_graph(self, limit=100):
        with self.driver.session() as session:
            ...
            for record in result:
                ...
                proteins[p["entry"]] = {"entry": p["entry"], "name": p["entry_name"]}
                if n:
                    proteins[n["entry"]] = {
                        "entry": n["entry"],
                        "name": n["entry_name"],
                    ...
                    edges.append(
                        {"source": p["entry"], "target": n["entry"], "weight": weight}
                    )
    ...
    def get_neighbors_by_id(self, entry_id):
        with self.driver.session() as session:
            ...
            return {
                "protein": {"entry": protein["entry"], "name": protein["entry_name"]},
                "neighbors": [
                    {"entry": n["entry"], "name": n["entry_name"]}
                    for n in neighbors
                    if n
                ],
                "edges": edge_list,
    ...
    def annotate_protein_multilabel(
        tx, entry_id, similarity_threshold=0.2, top_n=5, min_weight=0.0
    ):
        query = """
        ...
        filtered_ecs = [
            (ec, weight) for ec, weight in ec_weights.items() if weight >= min_weight
        ]
